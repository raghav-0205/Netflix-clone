...
const LoginSpace = ({useCase}) => {
    ...
    const checkUser = async (user) =>{
      try {
        const response = await axios.post('http://localhost:3000/auth/signin', user);
        setMessage(response.data.message);
        return response
        } catch (error) {
          console.error(error);
  ...
  return (
    <>
    <div className='flex flex-col justify-evenly items-center gap-2 min-h-[70vh] w-[25vw] bg-[rgba(0,0,0,0.7)] rounded-lg p-5  text-white'>
        <h2 className="text-3xl font-bold mt-4 mb-3 pl-5 self-baseline">{useCase}</h2>
        <form onSubmit={handleSubmit} className="flex w-[90%] flex-col space-y-4">
        {useCase === "Sign up" && (<input
            type="text"
            placeholder="Name "
            value={name}
      ...
      {useCase === "Sign in" && (
        <div className="flex items-center gap-2 text-gray-400 text-sm">
          <label htmlFor="rememberMe" className="flex items-center cursor-pointer">
            <input id="rememberMe" type="checkbox" className="hidden peer" checked={rememberMe} onChange={() => setRememberMe(!rememberMe)}/>
            <div className="w-5 h-5 flex items-center justify-center border border-gray-600 rounded-sm bg-[#0c0c0c] peer-checked:bg-white peer-checked:hover:bg-gray-300  peer-checked:border-white hover:border-white transition-[background-color] delay-200 ease-in-out">
              <MdCheck className={`${rememberMe ? 'opacity-100': 'opacity-0'} text-black transition delay-200`} size={15} />
            </div>
        ...
        {showMsg && <div className="w-full">
              <Msg msg={message} />
          </div>}
       {(useCase === "Sign in") ? ( <div className="mt-6 text-sm">
          <span className="text-gray-300">New to Netflix? </span>
          <NavLink to="/signup" className="text-white hover:underline font-semibold" >Sign up now</NavLink>
